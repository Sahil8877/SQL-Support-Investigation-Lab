...
**Importing via Workbench's Table Data Import Wizard.** Used the wizard rather than hand writing `CREATE TABLE` statements for each file, since it infers column names and types directly from the CSV headers and a data sample, then creates the table and loads the rows in one pass. Run once per CSV, four times in total, each following the same sequence: select the wizard from the schema's context menu, confirm or create the destination table, review the inferred column types, then execute.
